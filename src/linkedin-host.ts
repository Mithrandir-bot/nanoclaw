...
function checkAndIncrementSearchQuota():
  ...
  if (q.count >= SEARCH_DAILY_CAP) return { ok: false, remaining: 0 };
  q.count += 1;
...
async function runScript(script: string, args: object): Promise<SkillResult> {
  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timer);
      // runScript writes its JSON result to stdout, then exits 1 on internal
      // errors. Always prefer that JSON over the raw exit-code string.
      try {
        const lines = stdout.trim().split('\n').filter(Boolean);
        const last = lines[lines.length - 1];
        const parsed = JSON.parse(last);
        if (parsed && typeof parsed === 'object' && 'success' in parsed) {
          resolve(parsed as SkillResult);
          return;
        }
      } catch {
      ...
      if (code !== 0) {
        resolve({
          success: false,
          message: `LinkedIn script exited ${code}: ${(stderr || stdout).slice(0, 400)}`,
        });
        return;
      }
      resolve({
  ...
  switch (type) {
    case 'linkedin_list_recent_messages':
      result = await runScript('list_messages', {
        limit: (data.limit as number) ?? 15,
        unreadOnly: (data.unread_only as boolean) ?? false,
      });
      break;
